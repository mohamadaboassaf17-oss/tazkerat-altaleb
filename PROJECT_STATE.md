...
**Current status:** local-first build-out phase. Milestones M1 (foundation/PWA shell/DB schema) ≈95%, most of M2 (auth UI + onboarding SQL) ≈70%, M3 (content-hierarchy CRUD, version/outbox mutation pipeline, hierarchy RLS written in-repo but not yet pushed to cloud), M4 (notes data layer, editor UI, knowledge graph, notes RLS written in-repo but not yet pushed to cloud), and M5 (sync engine — conflict-guard migration, serializer, push/pull/engine trio + wiring; code-complete locally but the migration is not yet pushed to cloud) exist as real code; SRS/media/sharing/dashboard features remain Planned.
...
- **Still missing:** `tasks.md` M6–M10 — SRS scheduler + card UI, media upload + freeze policy, public sharing + anon RLS, Markdown/PDF export, real Dashboard, Arabic-normalized search (M9), production hardening. Also no `README.md`, and none of the M1–M5 migrations has ever been pushed to a hosted project.
- **Problems/constraints:** see §9 (no git repo, stray `index_out.html`, hosted Supabase project linked but migrations not pushed — proven empty 2026-08-25, cloud application status now Known-Blocked).
- **Rough completion level (approximation, not measured):** ~40–45% of MVP scope. Milestones: M1 ≈ 95% (all automatable checks pass; only physical-device A2HS tap-through on iOS/Android remains — needs real hardware + HTTPS host), M2 ≈ 70% (cloud/auth-provider config outstanding), M3 ≈ 90% locally (CRUD + mutation pipeline + RLS migration + tests + docs complete; cloud push and live-policy verification outstanding — D14), M4 ≈ 100% locally (data layer + editor UI + graph + notes RLS migration + tests + live E2E complete; cloud-facing verification blocked by D14 like M2/M3), M5 ≈ 90% locally (code-complete: conflict-guard migration + serializer + push/pull/engine trio + wiring + specs at 113/113 across 10 files; cloud push and live E2E sync verification blocked like M2/M3/M4), M6–M10 = 0%.
...
1. **M2 closeout — external Supabase configuration.** Done in code: screens, seeding SQL, `users` RLS. Remaining: enable Google OAuth provider + set redirect URLs in the Supabase dashboard, create/link a hosted project, run `supabase db push`. The CLI itself now works locally (`npx supabase`, devDependency v2.115.0 — the earlier download-throttle issue is moot), but no access token exists on the machine: the concrete blocker is a human `npx supabase login`. Hosted project `pyvskirousshlwsqtoro` is confirmed present yet proven empty of migrations (K2).
...
### Critical
- [ ] Run `npx supabase login` (human step — no access token exists on this machine; `npx supabase projects list` fails LegacyPlatformAuthRequiredError), then `npx supabase link --project-ref pyvskirousshlwsqtoro` and push **all five migrations** (`supabase db push`). The CLI itself now works locally as a devDependency (v2.115.0) — any earlier claim that CLI installation was blocked is removed. Proven blocking 2026-08-25: the hosted project has no migrations applied, so demo-seed output is invisible to clients (categories insert → 403 RLS deny-by-default) and every M1–M5 policy/guard stays repo-only; this subsumes the former M3-only and four-migration push bullets. Real credentials are confirmed present in the bare `.env` (project ref `pyvskirousshlwsqtoro`; `.env.local` holds placeholders only).
- [ ] Configure Supabase Auth in the dashboard: Google OAuth provider (**needs Google client credentials from the owner — provider is currently disabled**); redirect URLs incl. `<origin>/auth/callback`, `/update-password` (replace localhost-only URLs in `supabase/config.toml`, K6).
- [ ] Verify end-to-end live: fresh signup → confirmation email → login → seeded demo template visible immediately (closes final M2 checkbox in `tasks.md`); plus live E2E sync verification of the M5 engine against the hosted project once migrations are pushed (offline → edit → reconnect flow).
- [ ] Initialize a git repository and commit current state (repo is **not under version control** — highest operational risk).

### High
- [x] M5: push/pull sync engine driven by `outbox`/`sync_meta`; highest-`version`-wins conflicts; backoff; online/action/interval triggers — **delivered 2026-08-25** (see §3 rows and D19–D21); remaining work is cloud-gated (Critical above).
...
- Live E2E sync verification of the M5 engine against hosted project `pyvskirousshlwsqtoro` — blocked by the missing Supabase access token (human `npx supabase login` required before `db push`; migrations unapplied, K2).
